...
sqlite3 ../chatlog.db <<< 'select text from messages where text is not null and text != "" and text not like "/%" and src != 120400693;' | tee chatlog.txt | python3 ../util/truecaser.py -t truecase.txt
pv chatlog.txt | python3 ../util/truecaser.py truecase.txt | perl -p -e 's|^[^\n ]+] ||' | python3 logcutfilter.py | opencc -c t2s.json | awk '!seen[$0]++' | tee chatlogf.txt | sed 's/“//g;s/”//g;s/  / /g;s/ /\n/g' | awk '{seen[$0]++} END {for (i in seen) {if (seen[i] > 5) print i}}' > chatdict.txt
rm chatlog.txt
